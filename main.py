...
def _build_instructions() -> str:
    ...
    return (
        ...
        "- **How the suttas DEFINE a term (in their own words)** — e.g. "
        "\"how do the suttas define dukkha\", \"the canonical definition of "
        "viññāṇa\": use **`define_from_suttas`**. It surfaces the canon's own "
        "definitional formulas (Katamañca X? … ayaṁ vuccati X, adhivacana, "
        "Vinaya `nāma`), cited and ranked — the *doctrinal* definition, "
        "distinct from `get_word_definition`'s lexical/dictionary sense. "
        "Reach for it whenever the user wants a term's meaning FROM the "
        "suttas, even if you already recall where it is defined.\n"
